...
class Multiple_Sensor_Reading : public IMU_maths
{
	public:
		...
		void addNewSample(Sample& _s, int num_thread); // add a new sample coming from the thread numbre num_thread
		void closeThread(int num_thread); // close the thread number num_thread
